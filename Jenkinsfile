pipeline {
    ...
    stages {
        ...
        stage('Setup and build') {
            agent { label 'ubuntu && 24.04 && php8.2 && nodejs22' }
            environment {
        ...
        stage('Acceptance tests') {
            agent { label 'ubuntu && 24.04 && nodejs22' }
            environment {
            ...
            stages {
                stage('Setup') {
                    steps {
                        ...
                        sh label: 'Initialize playwright', script: 'npx playwright install --with-deps chromium'
                    }
                }
